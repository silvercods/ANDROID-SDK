package com.sapihost.test_integration_kotlin
...
//              ,theme
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if(requestCode == PAY_ACTIVITY_REQUEST) {
            when (resultCode) {
                sapihostActivity.RESULT_SUCCESS -> {}
                sapihostActivity.RESULT_CANCELLED -> {}
                sapihostActivity.RESULT_DECLINE -> {}
                sapihostActivity.RESULT_FAILED -> {}
            }
            val error = data?.getStringExtra(sapihostActivity.DATA_INTENT_EXTRA_ERROR)
            val token = data?.getStringExtra(sapihostActivity.DATA_INTENT_EXTRA_TOKEN)
        }
    }

    // Payment Info
    internal fun getPaymentInfoOnlyRequiredParams(): sapihostPaymentInfo {
        return sapihostPaymentInfo(
    ...
    internal fun getPaymentInfoAllParams(): sapihostPaymentInfo {
        return sapihostPaymentInfo(
            PROJECT_ID, // project ID that is assigned to you
            "internal_payment_id_1", // payment ID to identify payment in your system
            1999, // 19.99
    ...
    // Additional
    internal fun setDMSPayment(paymentInfo: sapihostPaymentInfo) {
        paymentInfo.setAction(sapihostPaymentInfo.ActionType.Auth)
    }

    internal fun setActionTokenize(paymentInfo: sapihostPaymentInfo) {
        paymentInfo.setAction(sapihostPaymentInfo.ActionType.Tokenize)
    }

    internal fun setActionVerify(paymentInfo: sapihostPaymentInfo) {
        paymentInfo.setAction(sapihostPaymentInfo.ActionType.Verify)
    }

    internal fun setToken(paymentInfo: sapihostPaymentInfo) {
        paymentInfo.token = "token"
    }

    internal fun setReceiptData(paymentInfo: sapihostPaymentInfo) {
        paymentInfo.receiptData = "receipt data"
    }

    // if you want to hide the saved cards, pass the value - true
    internal fun setHideSavedWallets(paymentInfo: sapihostPaymentInfo) {
        paymentInfo.hideSavedWallets = false
    }

    // For forced opening of the payment method, pass its code. Example: qiwi, card ...
    internal fun setForcePaymentMethod(paymentInfo: sapihostPaymentInfo) {
        paymentInfo.forcePaymentMethod = "card"
    }

    internal fun setRecurrent(paymentInfo: sapihostPaymentInfo) {
        val recurrentInfo = sapihostRecurrentInfo(
            "R", // type
            "20", // expiry day
            "11", // expiry month
            "2030", // expiry year
...
//        )
    ...
    internal fun setKnownAdditionalFields(paymentInfo: sapihostPaymentInfo) {
        paymentInfo.sapihostAdditionalFields = arrayOf(
            sapihostAdditionalField(sapihostAdditionalFieldEnums.AdditionalFieldType.customer_first_name, "Mark"),
            sapihostAdditionalField(sapihostAdditionalFieldEnums.AdditionalFieldType.billing_country, "US")
        )
    }

    // Setup 3D Secure parameters
    internal fun setupThreeDSecureParams(paymentInfo: sapihostPaymentInfo) {
        val threeDSecureInfo = sapihostThreeDSecureInfo()

        val threeDSecurePaymentInfo = sapihostThreeDSecurePaymentInfo()
        ...
        val threeDSecureGiftCardInfo = sapihostThreeDSecureGiftCardInfo()
        ...
        val threeDSecureCustomerInfo = sapihostThreeDSecureCustomerInfo()
        ...
        val threeDSecureAccountInfo = sapihostThreeDSecureAccountInfo() // object with account information on record with merchant
        ...
        val threeDSecureShippingInfo = sapihostThreeDSecureShippingInfo() // object that contains shipment details
        ...
        val threeDSecureMpiResultInfo = sapihostThreeDSecureMpiResultInfo() // object that contains information about previous customer authentication
        ...
        paymentInfo.sapihostThreeDSecureInfo = threeDSecureInfo
    }
}
